...
{
    private readonly ILogger<ChangeEventStatus> logger;
    ...
    public ChangeEventStatus(User user, List<Event> evenimente, string eventsPath, JsonSerializerOptions options)
    {
        ...
        logger = Program.LoggerFactory.CreateLogger<ChangeEventStatus>();
        ...
        logger.LogInformation("Change event status opened for {Username}", user.Username);
    ...
    private void LoadMyEvents()
    {
        ...
        if (myEvents.Count == 0)
        {
            logger.LogWarning("Organizer has no events");
    ...
    private void button1_Click(object? sender, EventArgs e)
    {
        ...
        logger.LogInformation("Event {EventName} status changed to {Status}", ev.EventName, newStatus);
    ...
    {
        logger.LogInformation("Change event status form closed");
        Close();
    }
}
